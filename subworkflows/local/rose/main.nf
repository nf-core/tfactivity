...
workflow ROSE {
    ...
    ch_filter_predictions = Channel.empty()
        ...
        .map { meta1, pred, _meta2, filtering -> [meta1, pred, filtering] }
    ...
    FILTER_OVERLAPS(TSS_OVERLAP.out.intersect, [], false)
    ...
    ch_subtract_overlaps = STITCHING.out.bed
        ...
        .filter { meta1, _stitched, meta2, _overlaps -> meta1.id == meta2.id }
    ...
    ch_unstitched_regions = FILTER_OVERLAPS.out.output
        .combine(ch_bed)
        .filter { meta1, _overlaps, meta2, _pred -> meta1.id == meta2.id }
    ...
    ch_concat_and_sort = SUBTRACT_OVERLAPS.out.bed
        ...
        .filter { meta1, _stitched, meta2, _unstitched -> meta1.id == meta2.id }
